...
const StoryReader: React.FC<Props> = ({
  ...
  // Reset slide index when new images are generated
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full max-w-7xl mx-auto gap-0 lg:gap-8 pb-32 lg:pb-10 relative">
      ...
      {/* Image Selection Modal - Slideshow Style */}
      {isSelectingImage && imageVariations && imageVariations.length > 0 && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1a1d] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
            {/* Modal Header */}
            <div className="p-4 border-b border-white/5 bg-gradient-to-r from-indigo-900/30 to-purple-900/30">
              ...
              <p className="text-xs text-gray-400 mt-1 font-serif">
                {currentSlideIndex + 1} / {imageVariations.length} 枚目 - 左右の矢印で切り替え、選択ボタンで確定
              </p>
            </div>
            ...
            <div className="flex-1 flex items-center justify-center p-4 relative">
              {/* Left Arrow */}
              <button
                onClick={() => setCurrentSlideIndex(prev => prev > 0 ? prev - 1 : imageVariations.length - 1)}
                className="absolute left-2 z-10 w-12 h-12 rounded-full bg-black/60 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-black/80 hover:border-purple-500 transition-all"
                aria-label="前の画像"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex-1 flex items-center justify-center max-h-[60vh]">
                <img
                  src={imageVariations[currentSlideIndex]}
                  alt={`バリエーション ${currentSlideIndex + 1}`}
              ...
              {/* Right Arrow */}
              <button
                onClick={() => setCurrentSlideIndex(prev => prev < imageVariations.length - 1 ? prev + 1 : 0)}
                className="absolute right-2 z-10 w-12 h-12 rounded-full bg-black/60 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-black/80 hover:border-purple-500 transition-all"
                aria-label="次の画像"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="p-4 border-t border-white/5 bg-[#151518]">
              <button
                onClick={() => onSelectImageVariation(imageVariations[currentSlideIndex])}
                className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold py-3 px-6 rounded-xl tracking-wider text-sm transition-all shadow-lg shadow-purple-500/20"
              >
                ✓ この画像を選択
              </button>
            </div>
