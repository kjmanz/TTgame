...
4. 必ずJSON形式で出力してください。
5. **【最重要・文字数厳守】出力テキストは「必ず3000文字以上」にしてください。これは絶対に守るべき最優先ルールです。**
6. **【警告】章やパートが進むにつれて文字数が減少する傾向があります。Part1でもPart20でも、常に同じ3000文字以上を維持してください。**
7. **文字数を稼ぐコツ**: 擬音を多用する、喘ぎ声を詳細に描写する、身体の反応を細かく描写する、心理描写を追加する。
